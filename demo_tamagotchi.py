#!/usr/bin/env python3
"""
Demo Last.fm Tamagotchi - Test version without Last.fm API
Simulates scrobbles to test the pet system
"""
...
class DemoTamagotchi:
    def __init__(self):
        ...
        # Demo tracks for simulation
        self.demo_tracks = [
            {'name': 'Bohemian Rhapsody', 'artist': {'#text': 'Queen'}},
            {'name': 'Hotel California', 'artist': {'#text': 'Eagles'}},
            {'name': 'Stairway to Heaven', 'artist': {'#text': 'Led Zeppelin'}},
            {'name': 'Imagine', 'artist': {'#text': 'John Lennon'}},
            {'name': 'Hey Jude', 'artist': {'#text': 'The Beatles'}},
            {'name': 'Smells Like Teen Spirit', 'artist': {'#text': 'Nirvana'}},
            {'name': 'Wonderwall', 'artist': {'#text': 'Oasis'}},
            {'name': 'Creep', 'artist': {'#text': 'Radiohead'}},
            {'name': 'Sweet Child O Mine', 'artist': {'#text': 'Guns N Roses'}},
            {'name': 'Nothing Else Matters', 'artist': {'#text': 'Metallica'}}
        ]
        
        # Initialize display
        self.root = tk.Tk()
        self.root.title("Last.fm Tamagotchi Demo")
        ...
        self.running = True
        self.demo_thread = threading.Thread(target=self.demo_scrobbles, daemon=True)
    ...
    def setup_demo_controls(self):
        ...
        # Feed button
        feed_btn = tk.Button(control_frame, text="🎵 Feed Pet", 
                           command=self.manual_feed, bg='#FF69B4', fg='white')
        feed_btn.pack(side=tk.LEFT, padx=5)
        
        # Level up button
        level_btn = tk.Button(control_frame, text="⬆️ Level Up", 
                            command=self.manual_level_up, bg='#FFD700', fg='black')
        level_btn.pack(side=tk.LEFT, padx=5)
        
        # Evolve button
        evolve_btn = tk.Button(control_frame, text="🌟 Evolve", 
                             command=self.manual_evolve, bg='#8A2BE2', fg='white')
        evolve_btn.pack(side=tk.LEFT, padx=5)
        
        # Reset button
        reset_btn = tk.Button(control_frame, text="🔄 Reset", 
                            command=self.reset_pet, bg='#FF6B6B', fg='white')
        reset_btn.pack(side=tk.LEFT, padx=5)
    
    def manual_feed(self):
        """Manually feed the pet"""
        track = random.choice(self.demo_tracks)
        self.feed_pet(track)
    
    def manual_level_up(self):
        """Manually level up the pet"""
        self.pet_state['experience'] = self.pet_state['level'] * 100
        self.level_up()
    
    def manual_evolve(self):
        """Manually evolve the pet"""
        self.evolve()
    ...
    def reset_pet(self):
        ...
        print("🔄 Pet reset to initial state!")
    
    def demo_scrobbles(self):
        """Simulate scrobbles for demo"""
        while self.running:
            # Randomly feed the pet every 10-30 seconds
    ...
    def draw_demo_info(self):
        ...
        # Demo title
        self.canvas.create_text(self.screen_width // 2, demo_y,
                              text="🎵 Last.fm Tamagotchi Demo", 
                              fill='#FF69B4', font=('Arial', 18, 'bold'))
        
        # Demo description
        self.canvas.create_text(self.screen_width // 2, demo_y + 30,
                              text="This is a demo version - use buttons above to test features", 
                              fill='#FFD700', font=('Arial', 12))
        
        # Instructions
        instructions = [
            "🎵 Feed Pet: Simulates a new scrobble",
            "⬆️ Level Up: Instantly levels up the pet",
            "🌟 Evolve: Triggers evolution",
            "🔄 Reset: Resets pet to initial state"
        ]
        
        for i, instruction in enumerate(instructions):
            y = demo_y + 60 + i * 20
            self.canvas.create_text(20, y, text=instruction, 
    ...
    def run(self):
        """Start the demo tamagotchi"""
        print("🎵 Starting Last.fm Tamagotchi Demo")
        print("Press ESC to exit")
        print("Use the buttons to test different features!")
        self.root.mainloop()
        self.running = False
